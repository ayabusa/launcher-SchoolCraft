...
{
    class Cracked_Launcher
    {
        public static void Main(string Username, string version)
        {
            Console.WriteLine(CmlLib._Test.tstr);
            var p = new Cracked_Launcher();
            ...
            session = p.OfflineLogin(Username); // Login by mojang email and password
        ...
        MSession OfflineLogin(string username)
        {
            // Create fake session by username
            return MSession.GetOfflineSession("tester123");
        }

        void Start(MSession session, string version)
        {
